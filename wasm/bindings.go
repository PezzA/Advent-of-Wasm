// Package wasm is a wrapper for wasm that assumes the containint web page has a single canvas which is used specifically for the wasm file.  The main
// reason for having this in a seperate package is to prevent editor fails when trying to understand the build targets for syscall/js.
package wasm
